...
export class FinishedListComponent  implements OnInit {
  ...
  loadGames() {
    this.gamesService.getUserGamesListByUserId(this.user.id, 'usersListCompleted').subscribe({
      next: (response: any) => {
...
/*   searchGameByTitle(){
  ...
  //api does not support partial matching, so the title had to be exactly the same as the one in the DB
    ...
    removeGameFromList(gameId:string, listName:string){
      ...
      this.gamesService.putGameToList(this.GamingList, 'usersListCompleted').subscribe({
        next: (response: any) => {
